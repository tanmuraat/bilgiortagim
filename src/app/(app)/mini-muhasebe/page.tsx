...
export default function MuhasebePage() {
  ...
  const [form, setForm] = useState({
    category: 'arac_kiralama', amount: '', description: '',
    transaction_date: format(new Date(), 'yyyy-MM-dd'), vehicle_id: '',
  })
  ...
  const fetchData = useCallback(async () => {
    ...
    const [txRes, rentalsRes, vehiclesRes] = await Promise.all([
      ...
      // Bekleyen tahsilatlar: ödenmemiş veya kısmi ödeme yapılmış tüm kiralamalar
      supabase.from('rentals').select('*, vehicles(plate, brand)')
        .eq('user_id', user.id)
    ...
    const overdueData = (rentalsRes.data || []).filter(r =>
      (Number(r.total_price || 0) - Number(r.paid_amount || 0)) > 0
    )
    ...
    // Bekleyen tahsilat: tüm ödenmemiş kiralamalar
    const pending = overdueData.reduce((s, r) => s + (Number(r.total_price || 0) - Number(r.paid_amount || 0)), 0)
    ...
    setPieData(Object.entries(expenseByCategory).map(([key, value]) => ({
      name: CATEGORY_LABELS[key] || key, value, color: EXPENSE_COLORS[key] || '#6B7280',
    })))
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      {/* Son İşlemler + Bekleyen Tahsilatlar */}
      <div className="grid grid-cols-3 gap-6">
        <div className="col-span-2 bg-[#141414] border border-[#2A2A2A] rounded-xl p-5">
        ...
        <div className="bg-[#141414] border border-[#2A2A2A] rounded-xl p-5">
          <div className="flex items-center gap-2 mb-4">
            ...
            {overdueRentals.length > 0 && (
              <span className="ml-auto bg-yellow-500/20 text-yellow-400 text-[10px] font-bold px-2 py-0.5 rounded-full">{overdueRentals.length}</span>
            )}
          </div>
          <div className="space-y-3 max-h-80 overflow-y-auto">
            {overdueRentals.length === 0 ? (
              <p className="text-gray-500 text-sm">Bekleyen tahsilat yok 🎉</p>
              ...
              return (
                <div key={r.id} className="bg-[#1E1E1E] border border-yellow-500/20 rounded-lg p-3">
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-white text-sm font-medium truncate">{r.customer_name}</span>
                    <span className="text-yellow-400 text-sm font-bold flex-shrink-0 ml-2">₺{remaining.toLocaleString('tr-TR')}</span>
                  </div>
                  <div className="text-gray-500 text-xs">{r.vehicles?.plate} · {r.vehicles?.brand}</div>
                  <div className="flex items-center justify-between mt-1">
                    <span className="text-gray-600 text-xs">Bitiş: {format(new Date(r.end_date), 'dd.MM.yyyy')}</span>
                    <span className={`text-[10px] px-1.5 py-0.5 rounded-full ${r.payment_status === 'partial' ? 'bg-orange-500/20 text-orange-400' : 'bg-red-500/20 text-red-400'}`}>
