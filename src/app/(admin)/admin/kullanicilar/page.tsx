...
import { Search, X, Eye, RefreshCw, FileText, Image } from 'lucide-react'
...
export default function KullanicilarPage() {
  ...
  useEffect(() => {
    ...
    if (search) result = result.filter(u =>
      (u.company_name + u.full_name + u.email + (u.phone || '')).toLowerCase().includes(search.toLowerCase())
    )
  ...
  const openDetail = async (user: any) => {
    ...
    setDocUrl(null)
    const { count } = await supabase.from('query_logs')
      .select('*', { count: 'exact', head: true }).eq('user_id', user.id)
    setUserQueryCount(count || 0)

    // Vergi levhası URL al
      ...
      try {
        if (user.tax_document_url.startsWith('http')) {
          ...
          const { data } = await supabase.storage
            .from('tax-documents')
            .createSignedUrl(user.tax_document_url, 3600)
  ...
  const autoRenewCount = users.filter(u => u.auto_renew && u.subscription_plan !== 'none').length
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex gap-3">
        <div className="relative flex-1">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Firma, isim, email veya telefon ara..."
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-[#2A2A2A]">
              {['Firma / Kullanıcı', 'Email', 'Telefon', 'Plan', 'Yenileme', 'Durum', 'Sub. Bitiş', 'Kayıt', ''].map(h => (
                ...
                <td className="px-4 py-3">
                  {u.subscription_end ? (
                    <div>
                      ...
                      {u.auto_renew && <div className="text-green-400 text-[10px] flex items-center gap-1"><RefreshCw size={8} /> Oto. yenilenir</div>}
                    </div>
            ...
            <div className="p-5 space-y-5">
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  { label: 'Otomatik Yenileme', value: selectedUser.auto_renew ? '✓ Aktif — Devamlı' : '✗ Kapalı — Takip Et' },
                  { label: 'Sub. Bitiş', value: selectedUser.subscription_end ? format(new Date(selectedUser.subscription_end), 'dd MMM yyyy', { locale: tr }) : '—' },
                  ...
                  <div key={i} className="bg-[#1E1E1E] rounded-lg p-3">
                    ...
                    <div className={`text-sm font-medium ${item.label === 'Otomatik Yenileme' ? (selectedUser.auto_renew ? 'text-green-400' : 'text-yellow-400') : 'text-white'}`}>
                      {item.value}
                    </div>
                  </div>
                ))}
                ...
                {!selectedUser.tax_document_url ? (
                  ...
                  <div className="bg-[#1E1E1E] border border-[#2A2A2A] rounded-xl overflow-hidden">
                    {isPdf ? (
                      <div className="p-5 text-center">
                        ...
                        <a href={docUrl} target="_blank" rel="noopener noreferrer"
                          ...
                          <Eye size={14} /> PDF'i Görüntüle (Yeni Sekmede)
                        </a>
                      </div>
                    ) : (
                      <div className="relative select-none">
                        ...
                        <div className="absolute inset-0 pointer-events-none flex items-center justify-center">
                          <div className="text-black/10 text-3xl font-bold rotate-45 select-none whitespace-nowrap">
                            BİLGİORTAĞIM · ADMİN GÖRÜNTÜLEMESİ
                          </div>
                        </div>
                      </div>
                    )}
                    <div className="bg-[#1A1A1A] px-3 py-2 flex items-center gap-2 border-t border-[#2A2A2A]">
                      ...
                      <span className="text-gray-500 text-xs">Vergi levhası — sadece admin görüntüleyebilir</span>
                    </div>
                  </div>
                ) : (
                  <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-xl p-4 text-center">
                    <p className="text-yellow-400 text-sm">Belge yüklenemedi.</p>
                    <button onClick={() => openDetail(selectedUser)} className="text-xs text-red-400 mt-1">Tekrar Dene</button>
                  </div>
                )}
              </div>
